...
import {
  ChevronLeft, Upload, ScanLine, Loader2,
  CheckCircle, Package, AlertTriangle, X
} from "lucide-react";
      ...
      {value ? (
        <div className="relative rounded-2xl overflow-hidden bg-surface-800 border border-brand-500/20">
          ...
          <div className="absolute bottom-2 left-2">
            <span className="tag bg-brand-500/10 text-brand-400 border border-brand-500/20 text-[11px]">
...
export default function ContributeProduct() {
  ...
  const runOCR = useCallback(async () => {
    ...
    setError("");
    try {
      const text = await extractText(ingredientImage);
      setForm((f) => ({ ...f, ingredients: text }));
      setOcrDone(true);
    } catch {
      setError("Could not read text from the image. You can type the ingredients manually.");
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { product } = await submitProduct({
        ...
        frontImageData:      frontImage      || undefined,
        ingredientImageData: ingredientImage || undefined,
      });
      ...
      try {
        ...
        navigate("/result", {
          state: {
            result:      { ...result, brandName: product.brandName, imageUrl: product.imageUrl },
            ingredients: product.ingredients,
          },
        });
      } catch {
      ...
      <div className="flex flex-col gap-5">
        ...
        <div>
          ...
          {ingredientImage && !ocrDone && (
            <button onClick={runOCR} disabled={isOCRing}
              ...
              {isOCRing
                ? <><Loader2 size={16} className="animate-spin" />Reading… {progress}%</>
                : <><ScanLine size={16} />Extract Ingredients from Image</>}
            </button>
          )}
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            {ocrDone && (
              <span className="text-[11px] font-mono text-brand-400/60 flex items-center gap-1">
                <CheckCircle size={10} /> OCR extracted — review and edit
              </span>
            )}
          </div>
