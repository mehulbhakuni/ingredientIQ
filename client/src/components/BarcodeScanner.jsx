...
import { Barcode, Loader2, X, ScanLine, RefreshCw, PackageSearch } from "lucide-react";
import { useBarcodeScanner } from "../hooks/useBarcodeScanner";
import { lookupBarcode } from "../utils/api";

// States: idle → scanning → detected → fetching → found | notFound | fetchError
export default function BarcodeScanner({ onProductReady, onSwitchToOCR }) {
  ...
  const [phase,   setPhase]   = useState("idle");     // idle | scanning | detected | fetching | found | notFound | fetchError
  ...
  if (phase === "idle") return (
    <div className="flex flex-col items-center justify-center gap-5 py-10">
      ...
      <button onClick={startScan} className="btn-primary w-auto px-10">
        Open Camera
      </button>
    </div>
  );
  ...
  if (phase === "notFound") return (
    <div className="flex flex-col items-center justify-center gap-5 py-8">
      ...
      <div className="text-center px-4">
        ...
        <p className="text-[12px] font-mono text-surface-200/40">
          Barcode <span className="text-surface-200/60">{barcode}</span> isn't in the database yet.
        </p>
      </div>
      <div className="flex flex-col gap-3 w-full px-2">
        <button onClick={onSwitchToOCR} className="btn-primary flex items-center justify-center gap-2">
          <ScanLine size={16} /> Scan Ingredients Instead
        </button>
        <button onClick={resetAll} className="btn-secondary flex items-center justify-center gap-2 text-[14px]">
          <RefreshCw size={14} /> Try Another Barcode
        </button>
      </div>
    </div>
  );
  ...
  if (phase === "found" && product) return (
    <div className="flex flex-col gap-4">
      ...
      {/* Actions */}
      <button
        onClick={() => onProductReady(product)}
